...
def validate_u29_list_deceased(individuals: Dict[str, Individual]) -> List[ErrorAnomaly]:
    out: List[ErrorAnomaly] = []
    for indi in individuals.values():
        if indi.death:
            dd = indi.death
            try:
                ...
                out.append(ErrorAnomaly(
                    ...
                    gedcom_line=indi.ged_line_start,
                    indi_or_fam_id=indi.id,
                    message=f"Deceased individual {indi.id}."))
            except:
                continue
    return out

def validate_u34_list_large_age_difference_between_married_couples(individuals: Dict[str, Individual], families: Dict[str, Family]) -> List[ErrorAnomaly]:
    out: List[ErrorAnomaly] = []
    for fam in families.values():
        if fam.married:
            md = fam.married
            try:
                mdp = _parse_date(md)
                husb = individuals.get(fam.husband_id)
                wife = individuals.get(fam.wife_id)
                husb_birth = _parse_date(husb.birthday)
                wife_birth = _parse_date(wife.birthday)
                hub_age_at_md = (mdp - husb_birth).days / 365
                wife_age_at_md = (mdp - wife_birth).days / 365
                if hub_age_at_md < 0:
                    continue
                if wife_age_at_md < 0:
                    continue
                if hub_age_at_md >= wife_age_at_md * 2:
                    out.append(ErrorAnomaly(
                        error_or_anomaly="INFO",
                        indi_or_fam="FAMILY",
                        user_story_id="US34",
                        gedcom_line=str(fam.ged_line_start),
                        indi_or_fam_id=fam.id,
                        message=f"Husband {fam.husband_id} ({hub_age_at_md}) was more than twice the age of Wife {fam.wife_id} ({wife_age_at_md}) when married."))
                if wife_age_at_md >= hub_age_at_md * 2:
                    out.append(ErrorAnomaly(
                        error_or_anomaly="INFO",
                        indi_or_fam="FAMILY",
                        user_story_id="US34",
                        gedcom_line=str(fam.ged_line_start),
                        indi_or_fam_id=fam.id,
                        message=f"Wife {fam.wife_id} ({wife_age_at_md}) was more than twice the age of Husband {fam.husband_id} ({hub_age_at_md}) when married."))
...
def main():
    ...
    OTHER_INFO.extend(validate_u29_list_deceased(individuals))
    OTHER_INFO.extend(validate_u34_list_large_age_difference_between_married_couples(individuals, families))
